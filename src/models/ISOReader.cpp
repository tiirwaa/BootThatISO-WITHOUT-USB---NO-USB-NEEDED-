...
bool GetIsoHandlerClsid(GUID &outClsid) {
    UInt32 num = 0;
    if (GetNumberOfFormats(&num) != S_OK)
        return false;
    ...
    if (findByName(L"Udf", outClsid))
        return true;
    if (findByName(L"Iso", outClsid))
        return true;
    if (findByName(L"Ext", outClsid))
...
public:
    explicit ExtractCallback(IInArchive *arc, const std::wstring &base,
        ...
        if (!_baseDir.empty()) {
            std::error_code ec;
            std::filesystem::create_directories(_baseDir, ec);
        }
    }
    ...
    STDMETHOD(SetOperationResult)(Int32) override {
        return S_OK;
    }
...
OpenResult OpenIsoArchive(const std::wstring &isoPath) {
    ...
    auto tryOpenWithClsid = [&](const GUID &clsid, CMyComPtr<IInArchive> &outArc, CMyComPtr<IInStream> &in) -> HRESULT {
        outArc.Release();
        if (CreateArchiver(&clsid, &IID_IInArchive, (void **)&outArc) != S_OK || !outArc)
            return E_FAIL;
    ...
    bool haveExt = false, haveUdf = false, haveIso = false;
    {
        ...
        if (GetNumberOfFormats(&num) == S_OK) {
            for (UInt32 i = 0; i < num; ++i) {
                NWindows::NCOM::CPropVariant prop;
                if (GetHandlerProperty2(i, NArchive::NHandlerPropID::kName, &prop) == S_OK && prop.vt == VT_BSTR &&
                    prop.bstrVal) {
                    std::wstring n(prop.bstrVal, prop.bstrVal + SysStringLen(prop.bstrVal));
                    NWindows::NCOM::PropVariant_Clear(&prop);
                    if (n == L"Ext" || n == L"Udf" || n == L"Iso") {
                        if (GetHandlerProperty2(i, NArchive::NHandlerPropID::kClassID, &prop) == S_OK &&
                            prop.vt == VT_BSTR && prop.bstrVal && SysStringByteLen(prop.bstrVal) == sizeof(GUID)) {
                            if (n == L"Ext") {
                                memcpy(&extClsid, prop.bstrVal, sizeof(GUID));
                                haveExt = true;
                            }
                            if (n == L"Udf") {
...
bool ISOReader::extractFiles(const std::string &isoPath, const std::vector<std::string> &filesInISO,
...
bool ISOReader::extractDirectory(const std::string &isoPath, const std::string &dirPathInISO,
                                 const std::string &destDir) {
    const std::wstring wIso   = Utf8ToWide(isoPath);
    auto               opened = OpenIsoArchive(wIso);
    if (!opened.ok)
