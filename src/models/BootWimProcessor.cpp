...
bool BootWimProcessor::mountAndProcessWim(const std::string &bootWimDest, const std::string &destPath,
    ...
    if (mountCode == 0) {
        ...
        if (integratePrograms) {
            eventManager_.notifyDetailedProgress(40, 100, "Integrando carpeta Programs en boot.wim");
            eventManager_.notifyLogUpdate("Iniciando integracion de carpeta Programs...\r\n");
            ...
            auto tryCopyPrograms = [&](const std::string &sourceDir) -> bool {
                ...
                eventManager_.notifyLogUpdate(
                    "Calculado tamaño de Programs: " + std::to_string(programsSize / (1024 * 1024)) + " MB\r\n");
                eventManager_.notifyDetailedProgress(0, 0, "Copiando carpeta Programs");

                if (fileCopyManager_.copyDirectoryWithProgress(sourceDir, programsDest, programsSize, copiedSoFar,
                                                               excludeDirs, "Copiando Programs")) {
                    logFile << ISOCopyManager::getTimestamp() << "Programs integrated into boot.wim successfully from "
                            << sourceDir << std::endl;
                    eventManager_.notifyLogUpdate("Carpeta Programs integrada exitosamente (" +
                                                  std::to_string(programsSize / (1024 * 1024)) + " MB)\r\n");
                    eventManager_.notifyDetailedProgress(0, 0, "");
                    return true;
                }
                logFile << ISOCopyManager::getTimestamp() << "Failed to integrate Programs into boot.wim from "
                        << sourceDir << std::endl;
                eventManager_.notifyLogUpdate("Error al integrar carpeta Programs\r\n");
                return false;
            };
            ...
            if (!programsIntegrated) {
                ...
                eventManager_.notifyLogUpdate("Extrayendo Programs del ISO...\r\n");
                if (isoReader_->extractDirectory(sourcePath, "Programs", programsDest)) {
                    long long extractedSize = Utils::getDirectorySize(programsDest);
                    copiedSoFar += extractedSize;
                    logFile << ISOCopyManager::getTimestamp()
                            << "Programs extracted from ISO and integrated into boot.wim" << std::endl;
                    eventManager_.notifyLogUpdate("Programs extraido del ISO e integrado exitosamente (" +
                                                  std::to_string(extractedSize / (1024 * 1024)) + " MB)\r\n");
                    programsIntegrated = true;
                } else {
                    logFile << ISOCopyManager::getTimestamp()
                            << "Programs directory not found in ISO or extraction failed" << std::endl;
                    eventManager_.notifyLogUpdate("Carpeta Programs no encontrada en el ISO\r\n");
                }
            }
        ...
        // We'll add "subst Y: X:\" to pecmd.ini so Y:\CustomDrivers will map to X:\CustomDrivers
        {
            ...
            auto tryCopyCustomDrivers = [&](const std::string &sourceDir) -> bool {
                ...
                eventManager_.notifyLogUpdate("Calculado tamaño de CustomDrivers: " +
                                              std::to_string(customDriversSize / (1024 * 1024)) + " MB\r\n");
                eventManager_.notifyDetailedProgress(0, 0, "Copiando CustomDrivers");
                ...
                if (fileCopyManager_.copyDirectoryWithProgress(sourceDir, customDriversStage.string(),
                                                               ...
                                                               "Copiando CustomDrivers")) {
                    logFile << ISOCopyManager::getTimestamp() << "CustomDrivers staged successfully from " << sourceDir
                            << std::endl;
                    eventManager_.notifyLogUpdate("CustomDrivers integrado exitosamente (" +
                                                  std::to_string(customDriversSize / (1024 * 1024)) + " MB)\r\n");
                    eventManager_.notifyDetailedProgress(0, 0, "");
                    return true;
                }
                logFile << ISOCopyManager::getTimestamp() << "Failed to stage CustomDrivers from " << sourceDir
                        << std::endl;
                eventManager_.notifyLogUpdate("Error al integrar CustomDrivers\r\n");
