...
DiskIntegrityChecker::DiskIntegrityChecker(EventManager* eventManager)
...
bool DiskIntegrityChecker::performDiskIntegrityCheck() {
    ...
    if (eventManager_) eventManager_->notifyLogUpdate("Verificando integridad del disco...\r\n");
    ...
    if (!CreatePipe(&hRead_chk, &hWrite_chk, &sa_chk, 0)) {
        if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo crear pipe para chkdsk.\r\n");
        return false;
    }
    ...
    if (!CreateProcessA(NULL, const_cast<char*>(cmd_chk.c_str()), NULL, NULL, TRUE, 0, NULL, NULL, &si_chk, &pi_chk)) {
        ...
        if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo ejecutar chkdsk.\r\n");
        return false;
    }
    ...
    if (eventManager_) eventManager_->notifyLogUpdate("Resultado de verificación de disco:\r\n" + Utils::ansi_to_utf8(output_chk) + "\r\n");
    ...
    if (exitCode_chk != 0) {
        ...
        if (eventManager_) eventManager_->notifyLogUpdate("Ejecutando chkdsk /f para reparar errores...\r\n");
        ...
        if (!CreatePipe(&hRead_stdin, &hWrite_stdin, &sa_f, 0)) {
            if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo crear pipe para stdin.\r\n");
            return false;
        }
        if (!CreatePipe(&hRead_f, &hWrite_f, &sa_f, 0)) {
            CloseHandle(hRead_stdin);
            CloseHandle(hWrite_stdin);
            if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo crear pipe para chkdsk /f.\r\n");
            return false;
        }
        ...
        if (!CreateProcessA(NULL, const_cast<char*>(cmd_f.c_str()), NULL, NULL, TRUE, 0, NULL, NULL, &si_f, &pi_f)) {
            ...
            CloseHandle(hRead_stdin);
            CloseHandle(hWrite_stdin);
            if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo ejecutar chkdsk /f.\r\n");
            return false;
        }
        ...
        while (true) {
            ...
            if (bytesAvailable > 0) {
                if (ReadFile(hRead_f, buffer_f, min(sizeof(buffer_f) - 1, bytesAvailable), &bytesRead_f, NULL) && bytesRead_f > 0) {
                    ...
                    if (eventManager_) eventManager_->notifyLogUpdate(buffer_f);
                    // Check if prompt is present and send input
                    if (!inputSent && output_f.find("(S/N)") != std::string::npos) {
                        if (eventManager_) eventManager_->notifyLogUpdate("Enviando respuesta 'S' a chkdsk...\r\n");
        ...
        if (eventManager_) eventManager_->notifyLogUpdate(loopEndMsg);
        ...
        if (eventManager_) eventManager_->notifyLogUpdate(processEndMsg);
        ...
        if (eventManager_) eventManager_->notifyLogUpdate(finalReadMsg);
        ...
        if (eventManager_) eventManager_->notifyLogUpdate(finalReadDoneMsg);
        ...
        if (eventManager_) eventManager_->notifyLogUpdate(verifyMsg);
        Sleep(100);
        ...
        if (scheduled) {
            ...
            if (eventManager_) eventManager_->notifyLogUpdate(scheduledMsg);
            ...
            if (!restarted) {
                ...
                if (eventManager_) eventManager_->notifyLogUpdate(failMsg);
                ...
                if (eventManager_) eventManager_->notifyLogUpdate(shutdownMsg);
            }
            ...
            if (eventManager_) eventManager_->notifyLogUpdate(notScheduledMsg);
        }
        logToGeneral("Fin de proceso de verificación/reinicio.\r\n");
        if (eventManager_) {
        ...
        if (exitCode_f != 0) {
            if (eventManager_) eventManager_->notifyLogUpdate("Error: Chkdsk /f falló.\r\n");
            return false;
        }
    }
    return true;
...
bool DiskIntegrityChecker::RestartComputer() {
    if (eventManager_) eventManager_->notifyLogUpdate("Intentando reiniciar el sistema...\r\n");

    // Enable shutdown privilege
    HANDLE hToken;
    TOKEN_PRIVILEGES tkp;
    if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
        if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo abrir el token del proceso.\r\n");
        return false;
    }
    ...
    if (!AdjustTokenPrivileges(hToken, FALSE, &tkp, 0, (PTOKEN_PRIVILEGES)NULL, 0)) {
        if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo ajustar los privilegios.\r\n");
        CloseHandle(hToken);
        return false;
    }
    if (GetLastError() != ERROR_SUCCESS) {
        if (eventManager_) eventManager_->notifyLogUpdate("Error: Falló la verificación de privilegios.\r\n");
        CloseHandle(hToken);
        return false;
    }
    CloseHandle(hToken);
    ...
    if (ExitWindowsEx(EWX_REBOOT, 0)) {
        ...
        if (eventManager_) eventManager_->notifyLogUpdate("Error: No se pudo reiniciar el sistema. Código de error: " + std::to_string(error) + "\r\n");
        return false;
    }
}
