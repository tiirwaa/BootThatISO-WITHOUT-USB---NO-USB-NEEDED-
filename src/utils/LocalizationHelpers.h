...
inline std::wstring LocalizedFormatW(const std::string& key, const std::vector<std::wstring>& args, const wchar_t* fallback = nullptr)
{
...
inline std::wstring LocalizedFormatW(const std::string& key, const std::initializer_list<std::wstring>& args, const wchar_t* fallback = nullptr)
{
    return LocalizedFormatW(key, std::vector<std::wstring>(args), fallback);
}

inline std::string LocalizedFormatUtf8(const std::string& key, const std::vector<std::wstring>& args, const char* fallback = nullptr)
{
    std::wstring formatted = LocalizationManager::getInstance().format(key, args);
    if (formatted.empty()) {
        return fallback ? std::string(fallback) : std::string();
...
inline std::string LocalizedFormatUtf8(const std::string& key, const std::initializer_list<std::wstring>& args, const char* fallback = nullptr)
{
